...
public class GuiCampFire extends GuiContainer {
	...
	protected void drawGuiContainerForegroundLayer(int x, int y) {
		String guiName = this.tileEntityCampFire.isInvNameLocalized() ? this.tileEntityCampFire.getInvName()
				: I18n.format(this.tileEntityCampFire.getInvName(), new Object[0]);

		this.fontRendererObj.drawString(guiName, this.xSize / 2 - this.fontRendererObj.getStringWidth(guiName) / 2, 6,
				4210752);
	...
	protected void drawGuiContainerBackgroundLayer(float f, int x, int y) {
		...
		if (this.tileEntityCampFire.isBurning()) {
			...
			if (this.tileEntityCampFire.cookTime > 0) {
