...
public class TileEntityNewFurnace extends TileEntity implements ISidedInventory {
	...
	public void readFromNBT(NBTTagCompound nbtTagCompound) {
		...
		this.temprature = nbtTagCompound.getShort("Temprature");
	...
	public void writeToNBT(NBTTagCompound nbtTagCompound) {
		...
		nbtTagCompound.setBoolean("Lit", (Boolean) this.isLit);
		nbtTagCompound.setShort("Temprature", (short) this.temprature);
	...
	public void updateEntity() {
		...
		if (!this.worldObj.isRemote) {
			if (this.furnaceBurnTime != 0 || this.slots[1] != null && this.isLit) {
				...
				if (this.isBurning() && this.canSmelt() || this.slots[1] != null && isLit) {
			...
			if (flag != this.furnaceBurnTime > 0 || getItemBurnTime(this.slots[1]) > 0 && isLit) {
				flag1 = true;
	...
	private boolean canSmelt() {
		if (this.slots[0] == null) {
			...
			ItemStack itemstack = NewFurnaceRecipes.smelting().getSmeltingResult(this.slots[0]);
			if (itemstack == null) {
				this.furnaceCookTime = 0;
				return false;
			}
