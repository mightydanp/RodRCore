...
public class RecipeHandler extends CRecipeHandler {
	...
	private static void campfireRecipes() {
		...
		registerCampfireRecipe(ModItems.unfired_clay_pan, 0, ModItems.fired_clay_pan, 0, 1, 0.0F, ItemReference.SMALLCRUCIBLE_NAME);
		registerCampfireRecipe(ModItems.unfired_clay_pot, 0, ModItems.fired_clay_pot, 0, 1, 0.0F, ItemReference.SMALLCRUCIBLE_NAME);
		registerCampfireRecipe(Blocks.sand, 1, Blocks.glass, 0, 1, 0.0F, ItemReference.SMALLCRUCIBLE_NAME);
	}
	/*
	 * (Set<Map.Entry<ItemStack, ItemStack>> set =
	 * FurnaceRecipes.smelting().getSmeltingList().entrySet();
	 * 
	 * for(Map.Entry<ItemStack, ItemStack> obj: set) { ItemStack input =
	 * (ItemStack)obj.getKey(); ItemStack output = (ItemStack) obj.getValue();
	 * 
	 * List<String> input_names = findAllOreNames(input); List<String>
	 * output_names = findAllOreNames(output);
	 * 
	 * System.out.println(input_names);
	 * 
	 * 
	 * for (String element : input_names) { System.out.println(element);
	 * if(element.startsWith("ore")){ registerNewFurnaceRecipe(input.getItem(),
	 * 0, output.getItem(), 0, 1, 0.0F, ""); } } }
	 * 
	 * registerNewFurnaceRecipe("", 0, "", 0, 1, 0.0F,
	 * ItemReference.SMALLCRUCIBLE_NAME);
	 * 
	 * 
	 * private static List<String> findAllOreNames(ItemStack input) { int[] ids
	 * = OreDictionary.getOreIDs(input); ArrayList<String> results = new
	 * ArrayList<String>(); for(int id : ids){ String name =
	 * OreDictionary.getOreName(id); results.add(name); } return results; }
	 */
